...
def fit_normalize(
    ...
    # Fill any remaining missing values with the column mean before scaling
    X = X.fillna(X.mean())
    ...
    # Preserve the original column order for the remaining columns
    original_cols = [c for c in df.columns if c not in features_to_drop]
    df_normalized = df_normalized[original_cols]
